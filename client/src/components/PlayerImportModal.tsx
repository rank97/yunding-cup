          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 items-center">
              ...
              <form
                ...
                className="lg:col-span-8 p-3 rounded-2xl bg-slate-950/60 border border-slate-800/80 flex flex-col sm:flex-row items-center gap-2.5"
              >
                <input
                  type="text"
                  ...
                  className="w-full sm:w-1/3 px-3.5 py-2 rounded-xl bg-slate-900 border border-slate-700 text-slate-100 text-sm focus:outline-none focus:border-purple-500 font-semibold disabled:opacity-50"
                />
                <input
                  type="text"
                  ...
                  className="w-full sm:w-1/2 px-3.5 py-2 rounded-xl bg-slate-900 border border-slate-700 text-slate-100 text-sm font-mono focus:outline-none focus:border-purple-500 font-medium disabled:opacity-50"
                />
                <button
                  type="submit"
                  disabled={isLocked || isFull || isAdding || !newName.trim()}
                  className="w-full sm:w-auto px-4 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-sm flex items-center justify-center gap-1.5 shrink-0 disabled:opacity-50 transition-all shadow-md shadow-purple-600/30"
                >
              ...
              <div className="lg:col-span-4 relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3.5 py-2 rounded-2xl bg-slate-950/60 border border-slate-800 text-slate-200 text-sm focus:outline-none focus:border-purple-500"
                />
              </div>
            </div>
              ...
              {filteredPlayers.length === 0 ? (
                ...
                filteredPlayers.map((p, idx) => {
                  ...
                  return (
                    <div
                      key={p.id}
                      className={`p-3.5 sm:p-4 px-4 sm:px-5 rounded-2xl border transition-all flex flex-col gap-3 ${
                        isEditing
                          ? 'bg-purple-950/30 border-purple-500/60 ring-1 ring-purple-500/40 shadow-lg'
                          : 'bg-slate-950/70 border-slate-800/80 hover:border-slate-700/80 hover:bg-slate-900/40 sm:flex-row sm:items-center sm:justify-between shadow-sm'
                      }`}
                    >
                      {/* Left: Seed + Avatar + Info or Inline Inputs */}
                      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3.5 min-w-0 flex-1 w-full">
                        {!isEditing && (
                          <div className="relative shrink-0">
                            <img
                              src={p.avatarUrl || defaultAvatar}
                              alt={p.name}
                              className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl object-cover border-2 border-slate-700/80 bg-slate-900 shadow-md"
                            />
                            <span className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-purple-600 text-white font-mono text-[10px] font-bold flex items-center justify-center shadow-md border border-slate-950">
                              #{p.initialSeed || idx + 1}
                            </span>
                          </div>
                        )}
                        ...
                        {isEditing ? (
                            ...
                            <div className="flex flex-col sm:flex-row items-center gap-3 flex-1 w-full">
                              ...
                              <div className="flex items-center gap-2.5 shrink-0">
                                <div
                                  ...
                                  className="relative cursor-pointer group/av rounded-2xl overflow-hidden ring-2 ring-purple-500 hover:ring-amber-400 transition-all shadow-md shrink-0"
                                  title="点击展开笑脸头像库选择"
                                >
                                  <img
                                    src={editAvatarUrl || defaultAvatar}
                                    alt="avatar"
                                    className="w-12 h-12 sm:w-14 sm:h-14 object-cover bg-slate-900 group-hover/av:scale-105 transition-transform"
                                  />
                                ...
                                <button
                                  ...
                                  className="px-2.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-purple-300 border border-slate-700 hover:border-purple-500/50 text-xs transition-all flex items-center gap-1.5 shrink-0 font-bold shadow-sm"
                                  title="随机换笑脸头像"
                                >
                              ...
                              <input
                                ...
                                placeholder="选手姓名"
                                className="w-full sm:w-1/3 px-3 py-2 rounded-xl bg-slate-900 border border-purple-400 text-slate-100 text-sm sm:text-base font-bold focus:outline-none"
                              />
                              <input
                                type="text"
                                value={editGameId}
                                ...
                                placeholder="游戏 ID"
                                className="w-full sm:w-1/2 px-3 py-2 rounded-xl bg-slate-900 border border-slate-700 text-slate-200 text-sm sm:text-base font-mono focus:outline-none focus:border-purple-400"
                              />
                            </div>

                            {/* Avatar Quick Picker Panel */}
                            {showAvatarPicker && (
                              <div className="p-3 bg-slate-900/98 border border-purple-500/50 rounded-2xl space-y-2 animate-in fade-in duration-150 shadow-xl">
                                <div className="flex items-center justify-between text-xs text-slate-400">
                                  <span className="font-bold text-amber-300 flex items-center gap-1.5">
                                    <Sparkles className="w-3.5 h-3.5 text-amber-400" />
                                    <span>点击精选笑脸头像即可直接选定：</span>
                                  </span>
                                  <button
                                    type="button"
                                  ...
                                  {PRESET_AVATARS.map((url, avIdx) => {
                                    ...
                                    return (
                                      <button
                                        ...
                                        className={`p-1 rounded-xl border transition-all hover:scale-110 ${
                                          isSelected
                                            ? 'border-amber-400 ring-2 ring-amber-400/60 bg-amber-400/20 shadow-md'
                          ...
                          <div className="min-w-0 flex-1 space-y-1">
                            <div className="font-extrabold text-base sm:text-lg text-slate-100 flex items-center gap-2 truncate tracking-wide">
                              <span>{p.name}</span>
                            </div>
                            <div className="text-xs sm:text-sm font-mono font-medium text-slate-400 flex items-center gap-1.5 truncate">
                              <Gamepad2 className="w-4 h-4 text-purple-400 shrink-0" />
                              <span>游戏 ID:</span>
                              <span className="text-slate-200 font-semibold">{p.gameId || '—'}</span>
                            </div>
                          </div>
                        )}
                      </div>
                        ...
                        {isEditing ? (
                          <>
                            <button
                              ...
                              disabled={isSaving}
                              className="px-4 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-sm flex items-center gap-1.5 transition-all shadow-md"
                            >
                            ...
                            <button
                              ...
                              className="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm font-medium transition-all"
                            >
                              取消
                            </button>
                          </>
                          ...
                          <>
                            <button
                              ...
                              className="px-3.5 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-700/80 hover:border-purple-500/60 text-slate-300 hover:text-purple-300 text-xs font-bold transition-all flex items-center gap-1.5 shadow-sm"
                              title="编辑选手姓名、ID与头像"
                            >
                            ...
                            {!isLocked && (
                              <button
                                ...
                                className="px-3.5 py-2 rounded-xl bg-rose-950/30 hover:bg-rose-900/50 border border-rose-900/40 hover:border-rose-700/60 text-rose-400 hover:text-rose-300 text-xs font-bold transition-all flex items-center gap-1.5 shadow-sm"
                                title="从名册中移除选手"
                              >
